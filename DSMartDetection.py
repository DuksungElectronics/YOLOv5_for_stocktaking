...
Usage - formats:
    $ python detect.py --weights yolov5s.pt                 # PyTorch
                                 ...
                                 yolov5s_paddle_model       # PaddlePaddle1
...
item_id_dict = {
    'jin': 1, 'sand': 2, 'kan': 3, 'chap': 4, 'fried': 5, 'shrimp': 6
}

item_loc = [4, 3, 1, 0, 5, 2]

parent_url = "http://localhost:8080/stock"
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # pred = utils.general.apply_classifier(pred, model, im, im0s)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                stockBuff.append(items)
                if (len(stockBuff) > stockBuffSize):
                    stockBuff.pop(0)
                stockBuffNum = np.array(stockBuff).T
                for idx, item in enumerate(stockBuffNum):
                    frequency = np.bincount(item)
                    stock = frequency.argmax()
                    item_stock_dict[item_id_dict[names[idx]]] = stock

                # Write results
                for *xyxy, conf, cls in reversed(det):
                    # xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                    # print(len(det[xywh[0] > 0.3]), len(det))
        ...
        if (frameCnt % updateInterval == 0):
            if(before_stock != str(item_stock_dict)[1:-1]):
                print(item_stock_dict)
                data = {
                    'item_stock_list': str(item_stock_dict)[1:-1]  # 중괄호 제거한 딕셔너리 문자열 표현
                }
                """
                try:
